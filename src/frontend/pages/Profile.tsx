            ...
            <div className="grid grid-cols-3 gap-2 md:gap-3">
              {(['light', 'dark', 'system'] as const).map((mode) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-2 py-4 px-2 rounded-3xl border-2 transition-all duration-300 ${
                    themeMode === mode 
             ...
             <div className="space-y-3">
                ...
                {userAddresses.map((addr: any) => (
                  <div key={addr.id} className={`p-4 rounded-[1.5rem] border relative group transition-all ${addr.id === 0 ? 'border-emerald-500/20 bg-emerald-50/10 dark:bg-emerald-900/5' : 'border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950/20 hover:border-emerald-500/30'}`}>
                    {addr.id === 0 && <div className="absolute -right-2 -top-2 px-3 py-1 bg-emerald-600 text-[8px] font-black text-white rounded-bl-xl uppercase tracking-tighter transform rotate-3">Signup Address</div>}
                        ...
                        onClick={async () => {
                          if(confirm('Delete this address?')) {
                            const res = await fetch(`/api/user/addresses/${addr.id}`, {
                              method: 'DELETE',
                              headers: { 'Authorization': user?.token || String(user?.id) }
                            });
                 ...
                 try {
                   const res = await fetch('/api/user/addresses', {
                     ...
                     headers: { 
                       ...
                       'Authorization': user?.token || String(user?.id)
                     },
