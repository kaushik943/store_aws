...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div
      className="bg-white dark:bg-slate-900 rounded-[1.25rem] border border-slate-100 dark:border-slate-800 group flex flex-col cursor-pointer transition-all duration-200 hover:shadow-xl hover:border-emerald-200 dark:hover:border-emerald-800 overflow-hidden w-full relative"
    ...
    >
      ...
      <div className="relative w-full aspect-square bg-white dark:bg-slate-800 flex items-center justify-center border-b border-slate-50 dark:border-slate-800/80 overflow-hidden">
        ...
        <button
          ...
          className="absolute top-2 right-2 w-7 h-7 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-full text-slate-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all border border-slate-100 dark:border-slate-700 hover:text-emerald-600 shadow-sm z-10"
          title="View Details"
        >
          <Eye size={13} />
        </button>
      ...
      {/* Content - Fixed Min Heights for Perfect Alignment */}
      <div className="p-3 flex flex-col flex-1 bg-white dark:bg-slate-900">
        {/* Enforce 2 lines max, ensure it always takes up exactly 2 lines height using min-h */}
        ...
        <div className="mt-auto flex items-center justify-between gap-1 pt-1">
          <div className="flex flex-col leading-none">
            ...
            {product.mrp && product.mrp > product.price ? (
              ...
              <span className="text-[9px] opacity-0">₹0</span> // Keeps height consistent even without mrp
            )}
          </div>

          {cartItem && cartItem.quantity > 0 ? (
            <div
              className="flex items-center bg-emerald-600 text-white rounded-lg overflow-hidden shrink-0"
            ...
            <button
              onClick={(e) => { e.stopPropagation(); addToCart(product); }}
              className="flex items-center gap-1 px-3 h-8 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400 rounded-lg font-black text-[10px] uppercase hover:bg-emerald-600 hover:text-white transition-all active:scale-95 shrink-0"
            >
